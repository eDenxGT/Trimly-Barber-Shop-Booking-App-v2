...
import {
  barberTopUpWallet,
  barberWithdrawFromWallet,
  getWalletPageDataForBarber,
  handleFailureBarberTopUpPayment,
  handleVerifyBarberTopUpPayment,
} from "@/services/barber/barberService";
import { useQueryClient } from "@tanstack/react-query";
import { useEffect, useState } from "react";

export function BarberWalletPage() {
  const { data, isFetching, isError } = useWallet(getWalletPageDataForBarber);
  const [isTopUpModalOpen, setIsTopUpModalOpen] = useState(false);
  const [isWithdrawModalOpen, setIsWithdrawModalOpen] = useState(false);
  const { successToast, errorToast } = useToaster();
  const queryClient = useQueryClient();
  const { setLoadingState } = useLoading();
  const { mutate: barberWithdraw } = useWithDrawMutation(
    barberWithdrawFromWallet
  );
  ...
  return (
    <div className="container mx-auto mt-16 py-8">
      ...
      <TopUpModal
        ...
        handleCreateOrder={barberTopUpWallet}
        handleFailure={handleFailureBarberTopUpPayment}
        handleVerifyPayment={handleVerifyBarberTopUpPayment}
